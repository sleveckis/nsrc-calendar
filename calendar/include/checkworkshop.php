...
function  checkforwserrors($ws_create_error_array, $ws_create_formvalues_array)
{
...
//
 ...
 if((!empty($ws_create_formvalues_array["country"])) &&  (!validcountrycode($ws_create_formvalues_array["country"])))
   {
    $ws_create_error_array["count"] = $ws_create_error_array["count"] + 1;
...
if(($ws_create_formvalues_array["begin_month"] == 'null') or
   ...
   ($ws_create_formvalues_array["end_year"] == 'null'))
    {
        $ws_create_error_array["count"] = $ws_create_error_array["count"] + 1;
        $ws_create_error_array["begin_date"] = "You must choose both a start and an end date for this workshop. At least one date item is from the drop-down lists is missing.";
